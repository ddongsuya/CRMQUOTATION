// 효력시험 110개 질환모델 프리셋 + 카테고리.
// 원본: design_handoff_efficacy_quotation/efficacy-models.js (코아스템켐온_신약개발지원본부_2025_유효성시험_국문.pdf)
// 데이터는 원본 그대로. 임의 수정 금지.

export type StudyModel = {
  id: string; title: string; titleEn: string; titleKr: string;
  categoryCode: string; category: string;
  species: string[]; speciesRaw: string; sex: string; ageWeeks: number;
  inductionMethod: string; durationWeeks: number | null; reportWeeks: number;
  evalItemsRaw: string; positiveControl: string;
  isInVitro: boolean; cellLine: string; cultureCondition: string;
  scheduleDurations: string[]; page: number;
};
export type StudyCategory = { code: string; name: string };
export type CategoryMeta = { code: string; name: string; nameEn: string; modelCount: number };
...
export function getModelsByCategory(category: string): StudyModel[] {
  return STUDY_MODELS.filter(m => m.category === category);
}

export function getModelById(id: string): StudyModel | undefined {
  return STUDY_MODELS.find(m => m.id === id);
}

export function getModelsByCategoryCode(code: string): StudyModel[] {
