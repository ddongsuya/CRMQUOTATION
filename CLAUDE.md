...
npm run test:rules           \# rules\_catalog.yaml 구조 검사
...
| missing\_info | 자동 결정 불가 → 사용자 확인이 필요한 항목 |
| SoT | Single source of Truth |
| 룰 타입 7종 | PF(가격공식) PR(선행) CG(조건부군) AD(추가옵션) WV(면제) SB(대체) GR(메타) — 총 33개 |
| 8단계 파이프라인 | filter → select → WV → SB → CG → PR → AD → PF → GR → 합계 \+ missing\_info |
...
- `docs/CRM_확장_설계.md` — CRM 데이터 모델·권한(MEMBER/TEAM\_LEAD/CENTER\_LEAD/ADMIN). **상태: draft, 코드 미반영 부분 있음**
